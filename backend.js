...
function uploadImage() {
    ...
    $.ajax({
        ...
        .done(function (data, statusText, xhr) {
            ...
            if (xhr.status == 200) loadBlobList();
        })
        .fail(function (xhr, status, error) {
            progressbar.progressbar("value", 100);
            progressLabel.text(
                "Error! " + xhr.responseText + " (Kode: " + xhr.status + ")"
            );
        });
}
...
function viewImage(blobIdx) {
    ...
    if (currentClass != null) currentClass.css("display", "none");
...
function analisaGambar(urlPath) {
    ...
    $.ajax({
        ...
        .done(function (data) {
            ...
            else analyzeResult.text("Tidak ada hasil analisa yang tersedia.");
        ...
        columnDefs: [{
            ...
            },
            {
                ...
                render: function (data, type, row, meta) {
                    ...
                    return (
                        '<div style="width:100%; height:100%; display: inline-flex; justify-content: center;"><a onclick="analyzeImage(' +
                        idx +
                        ')" id="simpleButton">Analisa</a>&emsp;<a onclick="viewImage(' +
                        idx +
                        ')" id="simpleButton">Lihat Gambar</a></div>'
                    );
                }
            }
        ]
